...
request_route {
        ...
        if (nat_uac_test(64)) {
                ...
                #        remove_hf("Contact");
                #        insert_hf("Contact: <sip:$fU@MY_DOMAIN>\r\n","Call-ID");
        ...
        if (is_method("REGISTER")) {
                route(REGISTRAR);
                xlog("L_NFO", "[REGISTER] tU: $tU $sht(vtp=>id_index::$tU) \n");
                if ($sht(vtp=>id_index::$tU) != $null) {
...
route[REGISTRAR] {
        xlog("L_INFO", "[REGISTRAR] tU: $tU fU: $fU \n");
        if (is_method("REGISTER")) {
...
route[SUSPEND] {
        if ( !t_suspend() ) {
                ...
                xlog("L_INFO","[SUSPEND] suspended transaction [$T(id_index):$T(id_label)] $fU=> $tU\n");
                xlog("L_INFO","[SUSPEND] suspended transaction [$T(id_index):$T(id_label)] $fU=> $tU\n");
                $sht(vtp=>id_index::$tU) = $T(id_index);
                $sht(vtp=>id_label::$tU) = $T(id_label);
                xlog("L_INFO","[SUSPEND] htable key value tU: $tU [$sht(vtp=>id_index::$tU) -- $sht(vtp=>id_label::$tU)]\n");
                route(SENDPUSH);
                exit;
        }
}
...
# USER location service
route[LOCATION] {
        xlog("L_INFO", "[LOCATION] rm=$rm ru=$ru du=$du tU=$tU  \n");
        if (!lookup("location")) {
                $var(rc) = $rc;
                t_newtran();
                switch ($var(rc)) {
...
route[SIPOUT] {
        xlog("L_INFO", "[SIPOUT] rm=$rm ru=$ru du=$du tU=$tU  uri \n");
...
route[BRIDGING] {
        xlog("L_INFO", "[BRIDGING] rm=$rm ru=$ru du=$du tU=$tU  \n");
        if (!has_totag()) {
...
onreply_route[MANAGE_REPLY] {
        xlog("L_INFO", "[MANAGE_REPLY] rm=$rm ru=$ru du=$du tU=$tU  \n");
...
onreply_route {
        xlog("L_INFO", "onreply_route Rp $Rp proto  WS \n");
